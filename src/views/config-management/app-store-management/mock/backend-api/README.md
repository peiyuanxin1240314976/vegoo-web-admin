...
| 连接异常列表（底部告警、弹窗下拉） | `10-credential-connection-anomalies.json` | `fetchAppStoreConnectionAnomalies` |

## 场景触发说明

| 页面场景     | 触发时机                           | 接口                           |
| ------------ | ---------------------------------- | ------------------------------ |
| 首屏加载     | `onMounted` 首次进入页面           | 表格 + 统计卡片 + 连接异常列表 |
...
| POST | `/credential/connection-anomalies`  | 连接异常列表 |
