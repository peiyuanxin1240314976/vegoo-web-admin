<template>
  <div class="monthly-ad-platform">
      ...
      <div class="platforms-grid">
        ...
        <div class="platform-card others-card" :style="{ '--accent': '#6b7280' }">
          ...
          <div v-for="p in otherPlatforms" :key="p.id" class="other-row">
            ...
            <span class="other-spend">{{ p.monthlySpend }}</span>
    ...
    <!-- ── 模块二：指标对比表 ─────────────────────────────────── -->
    <div class="data-card">
      <div class="card-title">月报广告平台指标对比</div>
...
<script setup lang="ts">
  ...
  const mainPlatforms = computed(() => {
    ...
    return adPlatformCards.slice(0, 5).map((p) => {
      ...
      return {
        ...p,
        color: accent,
        adSpend: '$' + (base * 30).toLocaleString('en-US'),
        acquisitions: (parseWanToNumber(p.acquisitions) * 30).toFixed(1) + '万',
        cpm: p.cpm ?? '$9.80',
        cpc: p.cpc ?? '$0.38',
        roi14d: p.roi14d ?? '93%'
      }
  ...
  const otherPlatforms = computed(() => {
    ...
    return adPlatformCards.slice(5).map((p) => {
      ...
      return {
        ...p,
        monthlySpend: '$' + (base * 30).toLocaleString('en-US')
      }
    })
  })

  const monthlyAdPlatformTableMock = [
    {
      id: 'google',
      name: 'Google',
      logo: 'G',
