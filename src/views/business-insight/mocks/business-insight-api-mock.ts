/**
 * 商业洞察 - IAA / IAP 分析 Mock 数据（与历史 src/api/business-insight.ts 行为一致）
 * 由 src/api/business-insight.ts 在「Mock 模式」下引用
 */
...
export function mockFetchIaaPlatformTabData(_params: IaaFilterState) {
...
export function mockFetchIaaPlacementTabData(_params: IaaFilterState) {
...
export function mockFetchIaaAdUnitTabData(_params: IaaFilterState) {
...
export function mockFetchIaaCountryTabData(_params: IaaFilterState) {
...
export function mockFetchIaaVersionTabData(_params: IaaFilterState) {
...
function iapDashMockNorm(v?: string) {
...
export function mockFetchIapOverviewTable(_params: {
  timeRange: string
  s_app_id?: string
  productType?: string
  s_country_code?: string
...
export function mockFetchIapOverviewTrend(params: IapOverviewDashParams & { timeRange: string }) {
  ...
  return Promise.resolve<IapOverviewTrend>({
    ordersRevenue: {
      ...
      orders: iapDashMockScaleNums([3200, 3500, 3800, 3600, 4100, 3900, 4200], s),
      revenue: iapDashMockScaleNums([620000, 680000, 710000, 690000, 750000, 730000, 780000], s)
    },
    conversion: {
      dates: ['2/4', '2/5', '2/6', '2/7', '2/8', '2/9', '2/10'],
      values: iapDashMockScaleNums([2.8, 3.0, 3.2, 3.1, 3.4, 3.3, 3.5], s * 0.98)
    },
    arpu: {
      dates: ['2/4', '2/5', '2/6', '2/7', '2/8', '2/9', '2/10'],
      values: iapDashMockScaleNums([11.2, 11.8, 12.0, 11.9, 12.2, 12.1, 12.5], s * 1.02)
    }
  })
}
...
export function mockFetchIapOverviewAppCards(
  ...
  let list = all
  if (plat === 'android') list = all.filter((c) => c.platform.toLowerCase() === 'android')
  else if (plat === 'ios') list = all.filter((c) => c.platform.toLowerCase() === 'ios')
  if (app)
    list = list.filter((c) => c.name.toLowerCase().replace(/\s/g, '') === app.replace(/\s/g, ''))
  if (!list.length) list = all.slice(0, 1)
...
export function mockFetchIapOverviewCountryDistribution(
  ...
  let list = base
  if (cc && cc !== 'ALL') {
    const one = base.filter((r) => r.s_country_code === cc)
    list = one.length ? one : base.slice(0, 1)
  }
  list = list.map((r) => ({
    ...r,
    orders: Math.max(1, Math.round(r.orders * scale))
  }))
  return Promise.resolve({ list })
}
...
export function mockFetchIapOverviewProductTypeDonut(
  ...
  if (pt === 'sub') {
    return Promise.resolve<{ list: IapProductTypeDonutItem[] }>({
      list: [
        { name: '内购', value: 5, percent: 5, amount: '$140K' },
        { name: '订阅', value: 95, percent: 95, amount: '$2.74M' }
      ]
    })
  }
  if (pt === 'iap') {
    return Promise.resolve<{ list: IapProductTypeDonutItem[] }>({
      list: [
        { name: '内购', value: 94, percent: 94, amount: '$2.05M' },
        { name: '订阅', value: 6, percent: 6, amount: '$130K' }
      ]
    })
  }
  const s = iapDashMockNumericScale(params)
  return Promise.resolve<{ list: IapProductTypeDonutItem[] }>({
    list: [
      {
        name: '内购',
        value: Math.round(42 * s),
        percent: Math.round(42 * s),
        amount: '$1.21M'
      },
      {
        name: '订阅',
        value: Math.min(100, 100 - Math.round(42 * s)),
        percent: Math.min(100, 100 - Math.round(42 * s)),
        amount: '$1.67M'
      }
    ]
  })
...
export function mockFetchIapOverviewPlatformCompare(
  ...
  const andBase = [22, 26, 30]
  if (plat === 'android') {
    return Promise.resolve<IapPlatformCompare>({
      dimensions: ['订单数', '收入', 'ARPU'],
      ios: iapDashMockScaleNums([1, 1, 1], s),
      android: iapDashMockScaleNums(andBase, s * 1.05)
    })
  }
  if (plat === 'ios') {
    return Promise.resolve<IapPlatformCompare>({
      dimensions: ['订单数', '收入', 'ARPU'],
      ios: iapDashMockScaleNums(iosBase, s * 1.05),
      android: iapDashMockScaleNums([1, 1, 1], s)
    })
  }
  return Promise.resolve<IapPlatformCompare>({
    dimensions: ['订单数', '收入', 'ARPU'],
    ios: iapDashMockScaleNums(iosBase, s),
    android: iapDashMockScaleNums(andBase, s)
  })
}
