<template>
  <div class="or-tab-content or-tab-organic">
    ...
    <div class="or-datasource-bar">
      ...
      数据来源：BigQuery 归因计算。该数据并不涉及到该用户人员的业务数据，通过规则计算用户量，国家级计划。最近更新：2026-03-11 08:00
    </div>
    ...
    <section class="or-kpi-grid">
      ...
      >
        ...
        <div
          v-if="card.trendText"
          class="or-kpi__trend"
          :class="card.trendUp === undefined ? 'trend-neutral' : card.trendUp ? 'trend-up' : 'trend-down'"
        >
    ...
    <section class="or-chart-grid">
      ...
      <ElCard class="or-panel" shadow="never">
        <template #header>自然量/流量来源分析</template>
      ...
      <!-- K-factor 趋势 -->
      <ElCard class="or-panel" shadow="never">
        <template #header>K-factor 趋势（30天）</template>
    ...
    <section class="or-bottom-grid">
      ...
      <ElCard class="or-panel" shadow="never">
        ...
        <ElTable :data="tabData?.kfactorChannels ?? []" size="small" class="kfactor-table" style="width: 100%">
          ...
          <ElTableColumn prop="kfactor" label="K-factor" min-width="75">
            <template #default="{ row }">
              <span :class="row.kfactor > 0.4 ? 'val-highlight' : ''">{{ row.kfactor.toFixed(2) }}</span>
            </template>
          </ElTableColumn>
