...
**筛选项**：① **公用** 应用、广告平台、国家等：`POST ${ANALYSIS_API_BASE}/user-growth/comprehensive-analysis/meta-filter-options`（`fetchComprehensiveAnalysisFilterOptions`，Mock 见 `comprehensive-analysis/config/data-source.ts`）。② **本页独有** **平台转化类型**（`PHONE_CALL_LEAD` 等，对应请求字段 `conversionType`）：**`08-meta-conversion-type-options.json`**。③ **映射状态** 等仍可 `mock/data.ts` 静态配置。提交筛选时 **string**，「全部」**`""`**（见 `backend-fields.mdc`）。
...
| **P0** | **转化类型筛选项（页面独有）** | `/api/v1/datacenter/analysis/user-growth/conversion-management/meta-conversion-type-options` | **08** |
| **P0** | 弹窗级联选项 | `/api/v1/datacenter/analysis/user-growth/conversion-management/meta-dialog-options` | 09 |
| **P0** | Data Tab KPI | `/api/v1/datacenter/analysis/user-growth/conversion-management/data-tab/overview-kpi` | **data-tab/01** |
| **P0** | Data Tab 树表 | `/api/v1/datacenter/analysis/user-growth/conversion-management/data-tab/table-rows` | **data-tab/02** |
| **P0** | Data Tab 右侧面板 | `/api/v1/datacenter/analysis/user-growth/conversion-management/data-tab/side-panels` | **data-tab/03** |
...
- **P0**：页面首屏或 Tab 切换必须的查询（列表、统计、筛选、Data Tab 拆分的 KPI/表/侧栏、详情等）。
...
| name Tab 筛选项 | `ConversionFilters` | 公用 meta + **`08-meta-conversion-type-options`** + `mock/data.ts`（状态等） | 转化类型走 **08**；应用/广告平台走公用 |
...
| data Tab 数据块 | `index.vue` `loadDataTab` | **`data-tab/01`～`03`**（或合并网关） | 当前 `fetchConversionDataMock` 内 `Promise.all` 对齐三契约；30s 轮询 |
...
#### 1. 映射列表 `POST /api/v1/datacenter/analysis/user-growth/conversion-management/mappings-list`

- **入参**：
  - `current?: number` — 页码，默认 1
  - `size?: number` — 每页条数，默认 20
  - `platform?: string` — `''` | `android` | `ios`
  - `appId?: string` — 应用（自有应用 ID，空为全部）
...
#### 2. 映射统计 `POST /api/v1/datacenter/analysis/user-growth/conversion-management/mappings-stats`

- **入参**：与列表筛选一致（无分页）：`platform`、`appId`、`conversionType`、`status`、`keyword`。
- **出参**：
...
#### 3. 弹窗级联选项 `POST /api/v1/datacenter/analysis/user-growth/conversion-management/meta-dialog-options`

- **入参**：
  - `source?: string`（或 `adPlatform`）— 广告平台枚举 **string**；`''` 表示不限；勿传 JSON number。
  - `mccAccount?: string`、`appId?: string` — 级联筛选（应用为自有 ID）
- **出参**：
...
#### 4. 映射详情 `POST /api/v1/datacenter/analysis/user-growth/conversion-management/mappings-detail`

- **入参**：`{ id: string }` — 映射记录 ID。
- **出参**：单条映射完整字段（id、platform、source、mccAccount、appId、conversionName、conversionId、platformConversionType、systemDisplayName、conversionDisplayType、billingType、status、remarks、createdAt、updatedAt 等），用于编辑回显。conversionId 为 string。

---

#### 5. 转化类型筛选项（页面独有）`POST /api/v1/datacenter/analysis/user-growth/conversion-management/meta-conversion-type-options`

- **入参**：无或 `{}`。
- **出参**：`conversionTypeOptions`: `{ label: string, value: string }[]`；`value` 为 `""`（全部）或 `PHONE_CALL_LEAD` / `DOWNLOAD` 等，与列表、data-tab 请求字段 **`conversionType`** 一致。
- **契约**：`08-meta-conversion-type-options.json`（与弹窗「转化展示分类」`13` **不同**，勿混用）。

---

#### 6. Data Tab（拆成 3 个 POST，筛选入参相同）

详见子目录说明 **`data-tab/README.md`**。

| 子能力 | URL（POST） | 契约文件 | 响应顶层 |
| --- | --- | --- | --- |
| 顶部 KPI | `.../conversion-management/data-tab/overview-kpi` | `data-tab/01-overview-kpi.json` | `kpi` |
| 三层树表 | `.../conversion-management/data-tab/table-rows` | `data-tab/02-table-rows.json` | `tableRows` |
| 右侧面板 | `.../conversion-management/data-tab/side-panels` | `data-tab/03-side-panels.json` | `sidePanels` |

- **入参（三者一致）**：`startDate`、`endDate`、`platform`、`appId`、`source`/`adPlatform`、`conversionType`；空字符串表示全部（应用筛选见 `backend-fields.mdc`「接口请求 · 应用筛选（appId）」）。
- **验收**：`tableRows` 根级 **≥3** 个 group；`level=conversion` 行 **`platformConversionType` 六类**在示例中写全；`sidePanels.typeDistribution` **六类**写全；树行 `trendPoints` 长度 **14**。
...
- **入参**：source、adPlatform、mccAccount、appId、conversionName、conversionId?、platformConversionType?、conversionDisplayType?、systemDisplayName、billingType?、status?、remarks?。
...
- **筛选入参类型**：与 `backend-fields.mdc` 一致，枚举/维度字段（`platform`、`conversionType`、`status`、`source` 等）均为 **string**；`sampleRequest` 勿写 number；「全部」用 `''`。
- **空/全选**：筛选字段用空字符串 `''` 表示全部；若后端用 null/undefined 需在实现中统一兼容。
- **应用维度**：**映射列表/统计/导出/批量**、**data-tab**、**data-export**、**弹窗级联 meta-dialog-options**、**创建/详情** 等请求与响应中的「应用」统一为 **`appId`**（string，自有应用 ID）。**兼容字段**：`source`（推荐）与 `adPlatform` 语义相同，后端优先取 `source`。
